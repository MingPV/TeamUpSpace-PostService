...
# Examples:
#  - Local ARM build: docker build --platform linux/arm64 -t mingpv/tus-post-service:latest .
#  - Multi-arch buildx & push: docker buildx build --platform linux/amd64,linux/arm64 -t <registry>/tus-post-service:TAG --push .

ARG GO_VERSION=1.24
ARG BINARY=tus-post-service
...
ENTRYPOINT ["/usr/local/bin/tus-post-service"]